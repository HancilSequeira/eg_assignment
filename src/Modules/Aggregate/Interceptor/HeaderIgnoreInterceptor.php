 ...
 *
 */
class HeaderIgnoreInterceptor extends ResponseHandler implements ApiInterceptorInterface
{
     ...
     * @return HeaderIgnoreRequest $response
     * 
     */
    public function prepareRequestDto(Request $request, $action = '')
    {
        try {
            ...
            $headerResponse = $this->serializer->deserialize(json_encode($requestObj), HeaderIgnoreRequest::class, 'json');

            return $headerResponse;
        } catch (\Exception $ex) {
            $this->logger->writeLog('ERROR', 'Exception while preparing dto; Error=>' . $ex->getMessage() . 'at line - ' . $ex->getLine() . ' in file' . $ex->getFile());
            return new HeaderIgnoreRequest();
        }
    }
    /**
     ...
     *
     * @param HeaderIgnoreRequest $requestObject
     * @return HeaderIgnoreRequest
     */
